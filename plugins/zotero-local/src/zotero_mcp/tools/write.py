...
def _add_by_arxiv(arxiv_id, collections, tags, write_zot, ctx,
    ...
    if isinstance(result, dict) and result.get("success"):
        ...
        # arXiv PDF attachment (respect attach_mode)
        pdf_url = f"https://arxiv.org/pdf/{arxiv_id}.pdf"
        pdf_status = "no PDF attached"
        if attach_mode == "linked_url":
            template_att = write_zot.item_template("attachment", "linked_url")
            template_att["url"] = pdf_url
            template_att["title"] = "PDF (linked URL)"
            template_att["contentType"] = "application/pdf"
            template_att["parentItem"] = item_key
            try:
                att_result = write_zot.create_items([template_att])
                if att_result.get("success"):
                    pdf_status = "PDF linked (URL only, no file upload)"
            except Exception as e:
                ctx.info(f"Linked URL attachment failed: {e}")
        elif attach_mode == "local":
            # Download PDF, create attachment record via API (no file upload),
            # place file in local Zotero storage for WebDAV sync
            try:
                pdf_resp = requests.get(pdf_url, timeout=30, stream=True, proxies={"http": None, "https": None})
                pdf_resp.raise_for_status()
                filename = f"arxiv_{arxiv_id.replace('/', '_')}.pdf"
                # Create attachment metadata record via Web API (no file)
                att_template = write_zot.item_template(
                    "attachment", "imported_file"
                )
                att_template["title"] = filename
                att_template["contentType"] = "application/pdf"
                att_template["filename"] = filename
                att_template["parentItem"] = item_key
                att_result = write_zot.create_items([att_template])
                if att_result.get("success"):
                    att_key = next(iter(att_result["success"].values()))
                    # Save PDF directly to local Zotero storage
                    storage_dir = os.path.expanduser(
                        f"~/Zotero/storage/{att_key}"
                    )
                    ...
                    with open(filepath, "wb") as f:
                        for chunk in pdf_resp.iter_content(chunk_size=8192):
                            f.write(chunk)
                    pdf_status = "PDF saved locally (WebDAV sync pending)"
            except Exception as e:
                ctx.info(f"Local PDF attachment failed (non-fatal): {e}")
                pdf_status = f"no PDF attached ({e})"
        elif attach_mode not in ("none",):
            try:
                pdf_resp = requests.get(pdf_url, timeout=30, stream=True, proxies={"http": None, "https": None})
                pdf_resp.raise_for_status()
